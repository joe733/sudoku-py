...
def solve_image(img, font_color=(255, 0, 0), font_path="FreeMono.ttf"):
    ...
    solutions = np.array(puzzle).reshape(81)
    paired = list(zip(solutions, warped_rgb_subd))
    img_solved = []
    for solution, img in paired:
...
def main(path, out_path, framerate, dims):
    ...
    while True:
        ...
        predicted, corners = solve_image(frame)
        warped_inverse = inverse_perspective(predicted, frame.copy(), np.array(corners))
